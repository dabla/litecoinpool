...
import org.smartwallet.stratum.HeadersStore;
...
public class ElectrumMultiWalletTest {
    ...
    private void supplyTransactionForAddress(String address, Transaction tx) throws IOException {
        ...
        ListenableFuture<StratumMessage> addressFuture = Futures.immediateFuture(aStratumMessage().withId(1L).withResult(historyResult).build());
        expect(client.call("blockchain.address.get_history", address)).andReturn(addressFuture);
        ListenableFuture<StratumMessage> txFuture = Futures.immediateFuture(aStratumMessage().withId(2L).withResult(Utils.HEX.encode(tx.bitcoinSerialize())).build());
    ...
    public void testHandleAddressQueueItem() throws Exception {
        ...
        multiWallet.handleAddressQueueItem(
        		aStratumMessage().withId(1L).withMethod("blockchain.address.subscribe").withParams(params).withResult(TextNode.valueOf("aaaa")).build());
        control.verify();
    }
