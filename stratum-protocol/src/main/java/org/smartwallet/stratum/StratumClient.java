...
import static org.smartwallet.stratum.StratumMethod.BLOCKCHAIN_ADDRESS_SUBSCRIBE;
import static org.smartwallet.stratum.StratumMethod.BLOCKCHAIN_HEADERS_SUBSCRIBE;
...
public class StratumClient extends AbstractExecutionThreadService {
    ...
    protected static Logger logger = getLogger(StratumClient.class);
    ...
    public StratumClient(NetworkParameters params, List<InetSocketAddress> addresses, boolean isTls) {
        this.params = params;
        serverAddresses = newArrayList((addresses != null) ? addresses : getDefaultAddresses());
    ...
    protected void createSocket() {
        ...
        lock.lock();
        try {
        	socket = new SocketFactory().create(isTls);
        } catch (IOException e) {
        ...
        private void doRun() throws InterruptedException, IOException {
            ...
            Futures.addCallback(future, new FutureCallback<StratumMessage>() {
                ...
                public void onSuccess(StratumMessage result) {
                    if (first) {
                        logger.info("server version {}", result.getResult());
                        peerVersion = result.getResult().asText();
                        first = false;
                    } else
                        logger.info("pong");
                }
    ...
    protected void runClient() throws IOException {
        ...
        while (true) {
            ...
            if (message.isResult())
                ...
                handleMessage(message.getMethod().toString(), message);
    ...
    public void call(String method, List<Object> params, boolean doQueue, SettableFuture<StratumMessage> future) {
        StratumMessage message = aStratumMessage()
        		.withId(currentId.getAndIncrement())
        		.withMethod(method)
        		.withParams(params)
        		.build();
        lock.lock();
        try {
            ...
            calls.put(message.getId(), new PendingCall(message, future));
    ...
    protected void handleResult(StratumMessage message) {
        PendingCall call = calls.remove(message.getId());
        if (call == null) {
            logger.warn("reply for unknown id {}", message.getId());
            return;
        }
        call.future.set(message);
        // Pass subscription result through queue also
        if (call.message.getMethod().isSubscribe()) {
            StratumMessage message1 =
            		aStratumMessage()
					.withMethod(call.message.getMethod())
					.withParams(call.message.getParams())
					.withResult(message.getResult())
					.build();
            handleMessage(message1.getMethod().toString(), message1);
        }
        if (call.message.getMethod().isGetHeader()) {
            StratumMessage message1 =
            		aStratumMessage()
					.withMethod(call.message.getMethod())
					.withParams(call.message.getParams())
    ...
    protected void handleMessage(String subscriptionMethod, StratumMessage message) {
        if (!subscriptions.containsKey(subscriptionMethod)) {
            logger.warn("message for unknown subscription {}", message.getMethod());
            return;
        }
        try {
            ...
            logger.warn("interrupted while handling message {}", message.getMethod());
        }
    }
    ...
    private void handleError(StratumMessage message) {
        ...
        call.future.setException(new StratumException(message.getError()));
    }
