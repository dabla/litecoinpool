...
import static org.smartwallet.stratum.StratumMethod.BLOCKCHAIN_GET_CHUNK;
import static org.smartwallet.stratum.StratumMethod.BLOCKCHAIN_GET_HEADER;
...
public class StratumChain extends AbstractExecutionThreadService {
    ...
    boolean handleBlock(StratumMessage item) {
        JsonNode result = item.getResult() != null ? item.getResult() : item.getParams().get(0);
        long height = result.get("block_height").longValue();
        if (item.getMethod().isHeadersSubscribe())
            peerHeight = height;
    ...
    private boolean handleChunk(StratumMessage item) {
        byte[] data = Utils.HEX.decode(item.getResult().asText());
