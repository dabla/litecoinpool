...
public class Crypto {
	...
	private Crypto(MessageDigest messageDigest, Mac mac) {
	...
	private static final ThreadLocal<Crypto> INSTANCE = new ThreadLocal<Crypto>() {
		...
		protected Crypto initialValue() {
			try {
				return new Crypto(MessageDigest.getInstance("SHA-256"),
	...
	public byte[] scrypt(byte[] value) throws GeneralSecurityException {
		return new Scrypt(value).digest();
	}
	
	private final class Scrypt {
	    ...
	    private Scrypt(byte[] header) {
			this.header = header;
	    }
	    
	    public byte[] digest() throws GeneralSecurityException {
	    	return digest(header[76] | header[77] << 8 | header[78] << 16 | header[79] << 24);
	    }
	    
	    public byte[] digest(int nonce) throws GeneralSecurityException {
	        int i, j, k;
