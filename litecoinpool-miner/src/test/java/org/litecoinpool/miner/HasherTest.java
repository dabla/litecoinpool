...
public class HasherTest {
	...
	public void hash_parallelIterable() throws Exception {
		...
		ParallelIterable.<Nonce>aParallelIterable().from(Nonce.values()).transform(hash(actual));
	}
	...
	public void hash_reactiveStreams() throws Exception {
		...
		Observable.fromIterable(Nonce.values()).flatMap(new io.reactivex.functions.Function<Nonce, ObservableSource<String>>() {
			@Override
	...
	private static Function<Nonce, String> hash(final Hasher hasher) {
		...
		return new Function<Nonce, String>() {
			...
			private String hash(final Hasher hasher, final TargetMatcher matcher, Nonce nonce) throws DecoderException {
